...
class BenchmarkTestDataset(data.Dataset):
    def __init__(
        self,
        dataroot_path: str,
        test_pair_file: str,
    ...
    def __getitem__(self, index):
        ...
        mask = Image.open(os.path.join(self.dataroot, "agnostic-mask", im_name.split('.')[0]+'_mask.png')).resize((self.width,self.height))
        mask = self.toTensor(mask)
        mask = mask[:1]
        mask = 1-mask
        im_mask = image * mask
        #print(os.path.join(self.dataroot, "densepose_color", im_name.split('.')[0] + '.png'))
        pose_img = Image.open(
            os.path.join(self.dataroot, "densepose_color", im_name.split('.')[0] + '.png')
        )
        pose_img = self.transform(pose_img)  # [-1,1]
            ...
            with torch.no_grad():
                for sample in test_dataloader:
                    ...
                    for i in range(len(images)):
                        ...
                        torchvision.utils.save_image(x_sample,os.path.join(args.output_dir, sample['im_name'][i].split('.')[0] + '_' + sample['c_name'][i]))
